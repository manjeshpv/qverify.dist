...
(function () {
  ...
  function CompanyComponent($log, QverifyConnection, $scope, $stateParams, $state) {
    ...
    $scope.gridOpts = {
      ...
      columnDefs: [{ name: 'Company', field: 'name' }, { name: 'Address', field: 'address' }, { name: 'Created on', field: 'created_on' }]
...
(function () {
  ...
  function UploadedComponent($log, QverifyConnection) {
    var LOG_TAG = 'UploadedComponent';
    var vm = this;
    var qverifyConnection = new QverifyConnection();
    qverifyConnection.fetchAllocationByStatus(1).then(function (allocations) {
      vm.Allocation = allocations;
  ...
  angular.module('appApp').component('uploaded', {
    templateUrl: 'app/routes/uploaded/uploaded.html',
    controller: UploadedComponent,
    controllerAs: 'Uploaded'
  });
})();
...
$templateCache.put("app/routes/uploaded/uploaded.html","<div ng-controller=\"NavbarController\" class=\"navbar navbar-default navbar-static-top\"><div class=\"container\"><div class=\"navbar-header\"><button type=\"button\" ng-click=\"nav.isCollapsed = !nav.isCollapsed\" ui-sref=\"login\" class=\"navbar-toggle\">Logout</button></div></div><div ui-view=\"\" class=\"col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main\"><div class=\"col-md-12 col-sm-12 col-xs-12 panel panel-info no-padder\"><div class=\"panel-heading font-bold h4 text-ellipsis bg-light\"><div class=\"fa fa-mouse-pointer text-dark\"></div>&nbsp;&nbsp;Allocated Case<!--i.fa.fa-angle-right.pull-right.font-bold--></div><div class=\"panel-body p-5px\"><scrollable-table watch=\"Dashboard.summary.applicantData\"><table class=\"table table-striped table-bordered m-b\"><thead><tr><th class=\"font-bold\">Case</th><th style=\"width:25%\" class=\"font-bold\">Allocated date</th></tr></thead><!--(ng-if=\'Dash.pipeline.length>0\')--><tbody><tr ng-repeat=\"allocation in Uploaded.Allocation\" ui-sref=\"candidate({case_id: allocation.cases_id })\"><td><a class=\"text-ellipsis\">{{allocation.Case.name}} ({{allocation.Case.CaseType.name}})</a></td><td><span YYYY-MM-DD=\"YYYY-MM-DD\" class=\"text-ellipsis moment format\">{{allocation.Case.created_at}}</span></td></tr></tbody></table></scrollable-table></div></div></div></div>");
