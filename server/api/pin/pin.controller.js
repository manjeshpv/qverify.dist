...
function index(req, res) {
  return _sqldb.Pin.findAll().then(respondWithResult(res)).catch(handleError(res));
}

// Gets a single Pin from the DB
function show(req, res) {
  return _sqldb.Pin.find({
    where: {
      _id: req.params.id
    }
...
function create(req, res) {
  return _sqldb.Pin.create(req.body).then(respondWithResult(res, 201)).catch(handleError(res));
}
